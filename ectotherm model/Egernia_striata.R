...
microin<-"C:/git/Egernia/microclimates/130_21/" # subfolder containing the microclimate input data
...
shdburrow<-0 #
mindepth<-1 # minimum depth (soil node) to which animal can retreat if burrowing
...
debpars<-as.data.frame(read.csv('../Egernia/DEB models/DEB_pars_Egernia_striata.csv',header=FALSE))$V1
fract<-1
f<-1.
...
X<-10 # food density J/cm2, approximation based on 200 Tetragonia berries per 1m2 (Dubas and Bull 1990) assuming energy content of Lilly Pilly (http://www.sgapqld.org.au/bush_food_safety.pdf)
